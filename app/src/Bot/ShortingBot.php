...
class ShortingBot implements BotInterface
{
    ...
    public function run(): void
    {
        ...
        [$base, $quote] = WexnzClient::splitPair($this->pair);
    ...
    protected function processQuoteFunds(ShortingStrategy $strategy, string $base, string $quote)
    {
        ...
        if ($strategy->isBalanceAllowTrading($quote)) {
            ...
            $ticker = $strategy->getClient()->shared()->ticker($this->pair, true);
...
//                self::SCALE
//            );
            $allowedPrice = bcmul($priceDiff, 0.50, self::SCALE) + $ticker->getLow();
            ...
            $price = $currentPrice + $increaseUnit;
//            $price = bcadd((string)$currentPrice, (string)$increaseUnit, self::SCALE);
            $this->output->writeln("\t<w>My price: {$price}</w>");

            if ($price <= $allowedPrice) {
                $balance = $strategy->getClient()->getCurrencyFunds($quote);
                $volume = bcdiv($balance, $price, self::SCALE);
                ...
                $order = $strategy
                    ->getClient()
                    ->trade()
                    ->trade($this->pair, Api::BUY, $price, $volume, true);
                $this->output->writeln("\t<g>BUY order created.</g>");
                $this->output->writeln("\t\t<w>Id: {$order->getOrderId()}</w>");
                $this->output->writeln("\t\t<w>Received: {$order->getReceived()}</w>");
                $this->output->writeln("\t\t<w>Remains: {$order->getRemains()}</w>");
            } else {
    ...
    protected function processBaseFunds(ShortingStrategy $strategy, string $base, string $quote)
    {
        ...
        if ($strategy->isBalanceAllowTrading($base)) {
            ...
            $price = $latestBuy->getRate() + $margin;
//            $price = bcadd($latestBuy->getRate(), $margin, self::SCALE);
            $this->output->writeln("\t<w>Sell price: {$price} {$quote}</w>");
            try {
                $balance = $strategy->getClient()->getCurrencyFunds($base);
            } catch (ClientException $e) {
            ...
            $receive = bcmul($balance, $price, self::SCALE);
            $this->output->writeln("\t<w>Sell volume: {$balance} {$base}</w>");
            ...
            $order = $strategy
                ->getClient()
                ->trade()
                ->trade($this->pair, Api::SELL, $price, $balance, true);
